...
var pluginSetupWizardGui_createPluginSetupWizard = function createPluginSetupWizard(appendTarget) {
  ...
  var handleGenericError = function handleGenericError(success) {
    return function () {
  ...
  $wizard.on('click', '.nav>li>a', function () {
    ...
    var activateClicked = function activateClicked() {
      if (!$li.is('.active')) {
  ...
  var getJenkinsVersionFull = function getJenkinsVersionFull() {
  ...
  var getJenkinsVersion = function getJenkinsVersion() {
  ...
  var setPanel = function setPanel(panel, data, onComplete) {
    var decorate = function decorate($base) {
  ...
  var getInstallingPlugin = function getInstallingPlugin(plugName) {
  ...
  var setFailureStatus = function setFailureStatus(plugData) {
  ...
  var getAllDependencies = function getAllDependencies(pluginName, deps) {
    if (!deps) {
      // don't get stuck
      deps = [];
  ...
  var initInstallingPluginList = function initInstallingPluginList() {
    installingPlugins = [];
  ...
  var installPlugins = function installPlugins(pluginNames) {
  ...
  var toggleDependencyList = function toggleDependencyList() {
  ...
  var installDefaultPlugins = function installDefaultPlugins() {
    loadPluginData(function () {
      installPlugins(pluginManager["a" /* default */].recommendedPluginNames());
    });
  };

  var enableButtonsAfterFrameLoad = function enableButtonsAfterFrameLoad() {
  ...
  var enableButtonsImmediately = function enableButtonsImmediately() {
    jquery_default()('button').prop({
      disabled: false
    });
  ...
  var displayErrors = function displayErrors(iframe, errors) {
    if (!errors) {
      return;
    }
  ...
  var setupFirstUser = function setupFirstUser() {
    setPanel(firstUserPanel_default.a, {}, enableButtonsAfterFrameLoad);
  };

  var showConfigureInstance = function showConfigureInstance(messages) {
    setPanel(configureInstance_default.a, messages, enableButtonsAfterFrameLoad);
  };

  var showSetupCompletePanel = function showSetupCompletePanel(messages) {
  ...
  var stateHandlers = {
    ...
    CREATE_ADMIN_USER: function CREATE_ADMIN_USER() {
      setupFirstUser();
    },
    CONFIGURE_INSTANCE: function CONFIGURE_INSTANCE() {
      showConfigureInstance();
    },
    RUNNING: function RUNNING() {
      showSetupCompletePanel();
    },
    INITIAL_SETUP_COMPLETED: function INITIAL_SETUP_COMPLETED() {
      showSetupCompletePanel();
    },
    INITIAL_PLUGINS_INSTALLING: function INITIAL_PLUGINS_INSTALLING() {
      showInstallProgress();
    }
  };

  var showStatePanel = function showStatePanel(state) {
    if (!state) {
  ...
  var showInstallProgress = function showInstallProgress() {
  ...
  var finishInstallation = function finishInstallation() {
    closeInstaller();
  }; // load the plugin data, callback


  var loadPluginData = function loadPluginData(oncomplete) {
  ...
  var loadPluginCategories = function loadPluginCategories(oncomplete) {
    loadPluginData(function () {
      categories = [];
  ...
  var loadCustomPluginPanel = function loadCustomPluginPanel() {
  ...
  var pluginSelectionPanelData = function pluginSelectionPanelData() {
    return {
  ...
  var removePlugin = function removePlugin(arr, item) {
  ...
  var addPlugin = function addPlugin(arr, item) {
    arr.push(item);
  }; // refreshes the plugin selection panel; call this if anything changes to ensure everything is kept in sync


  var refreshPluginSelectionPanel = function refreshPluginSelectionPanel() {
  ...
  var walk = function walk(elements, element, text, xform) {
    var i,
        child,
  ...
  var findElementsWithText = function findElementsWithText(ancestor, text, xform) {
    var elements = [];
  ...
  var scrollPlugin = function scrollPlugin($pl, matches, term) {
    if (matches.length > 0) {
      if (lastSearch !== term) {
        findIndex = 0;
      } else {
  ...
  var searchForPlugins = function searchForPlugins(text, scroll) {
  ...
  var toggleSelectedSearch = function toggleSelectedSearch() {
  ...
  var toggleInstallDetails = function toggleInstallDetails() {
  ...
  var handleFirstUserResponseSuccess = function handleFirstUserResponseSuccess(data) {
    if (data.status === 'ok') {
      showStatePanel();
    } else {
      setPanel(errorPanel_default.a, {
  ...
  var handleFirstUserResponseError = function handleFirstUserResponseError(res) {
  ...
  var saveFirstUser = function saveFirstUser() {
    jquery_default()('button').prop({
      disabled: true
    });
  ...
  var skipFirstUser = function skipFirstUser() {
    jquery_default()('button').prop({
      disabled: true
    });
  ...
  var handleConfigureInstanceResponseSuccess = function handleConfigureInstanceResponseSuccess(data) {
    if (data.status === 'ok') {
      if (firstUserSkipped) {
  ...
  var handleConfigureInstanceResponseError = function handleConfigureInstanceResponseError(res) {
    // We're expecting a full HTML page to replace the form
    // We can only replace the _whole_ iframe due to XSS rules
    // https://stackoverflow.com/a/22913801/1117552
    var responseText = res.responseText;
  ...
  var saveConfigureInstance = function saveConfigureInstance() {
    jquery_default()('button').prop({
      disabled: true
    });
  ...
  var skipFirstUserAndConfigureInstance = function skipFirstUserAndConfigureInstance() {
    firstUserSkipped = true;
    skipConfigureInstance();
  };

  var skipConfigureInstance = function skipConfigureInstance() {
    jquery_default()('button').prop({
      disabled: true
    });
    var message = '';
  ...
  var saveProxyConfig = function saveProxyConfig() {
  ...
  var retryFailedPlugins = function retryFailedPlugins() {
  ...
  var continueWithFailedPlugins = function continueWithFailedPlugins() {
  ...
  var resumeInstallation = function resumeInstallation() {
    // don't re-initialize installing plugins
    initInstallingPluginList = function initInstallingPluginList() {};
  ...
  var restartJenkins = function restartJenkins() {
    pluginManager["a" /* default */].restartJenkins(function () {
      ...
      var pingUntilRestarted = function pingUntilRestarted() {
        pluginManager["a" /* default */].getRestartStatus(function (restartStatus) {
  ...
  var closeInstaller = function closeInstaller() {
  ...
  var bindClickHandler = function bindClickHandler(cls, action) {
  ...
  var actions = {
    ...
    '.install-home': function installHome() {
      showStatePanel();
    },
    '.install-selected': function installSelected() {
      installPlugins(selectedPluginNames);
    },
    '.toggle-install-details': toggleInstallDetails,
    '.install-done': finishInstallation,
    '.plugin-select-all': function pluginSelectAll() {
      selectedPluginNames = allPluginNames;
      refreshPluginSelectionPanel();
    },
    '.plugin-select-none': function pluginSelectNone() {
      selectedPluginNames = [];
      refreshPluginSelectionPanel();
    },
    '.plugin-select-recommended': function pluginSelectRecommended() {
    ...
    '.skip-plugin-installs': function skipPluginInstalls() {
      installPlugins([]);
    },
  ...
  if ('undefined' !== typeof setupWizardExtensions) {
    jquery_default.a.each(setupWizardExtensions, function () {
      this.call(self, {
        ...
        setPanel: setPanel,
        addActions: function addActions(pluginActions) {
          jquery_default.a.extend(actions, pluginActions);
        },
        addStateHandlers: function addStateHandlers(pluginStateHandlers) {
          jquery_default.a.extend(stateHandlers, pluginStateHandlers);
        },
        translationOverride: function translationOverride(it) {
          extensionTranslationOverrides.push(it);
        },
        setSelectedPluginNames: function setSelectedPluginNames(pluginNames) {
  ...
  var showInitialSetupWizard = function showInitialSetupWizard() {
...
/* harmony default export */ var pluginSetupWizardGui = ({
  init: pluginSetupWizardGui_createPluginSetupWizard
});
